...
const Prospectus = () => {
    ...
    return (
        <div className='bg-backthird m-2 md:m-10 mt-24 p-2 md:p-10 rounded-3xl'>
          <h1 className='text-white p-11 bg-slate-600 rounded-3xl'>Prospectus </h1>
          <div className='flex ...'>
            {article.map(articles => (
              <div key={articles.id} className="max-w-sm bg-white md:m-5 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <Link to="#">
                  <img className="rounded-t-lg w-100 h-100" src={logo} alt="" />
                </Link>
                <div className="p-5">
                  <Link to="#">
                    <p className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{articles.nomArticle}</p>
                  </Link>
